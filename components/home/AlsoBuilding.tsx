        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1px", background: "var(--c-border)" }} id="also-grid">
          {items.map(item => (
            <a key={item.name} href={item.href} target="_blank" rel="noopener noreferrer"
              style={{ display: "block", padding: "2rem 2.25rem", background: "var(--c-surface)", textDecoration: "none", transition: "background 0.22s" }}
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "0.85rem" }}>
                <div>
                  <p style={{ fontSize: "0.62rem", fontWeight: 600, letterSpacing: "0.16em", textTransform: "uppercase", color: "var(--c-accent)", marginBottom: "0.35rem" }}>{item.label}</p>
                  <p style={{ fontFamily: "var(--font-serif)", fontSize: "1.3rem", color: "var(--c-ink)" }}>{item.name}</p>
                </div>
                <ArrowUpRight size={16} color="var(--c-ink-muted)" />
              </div>
              <p style={{ fontSize: "0.875rem", color: "var(--c-ink-muted)", lineHeight: 1.7, marginBottom: "1rem" }}>{item.desc}</p>
              <span style={{ fontSize: "0.72rem", fontWeight: 600, color: "var(--c-ink-muted)", letterSpacing: "0.05em", borderBottom: "1px solid var(--c-border)", paddingBottom: "1px" }}>{item.tag} ↗</span>
            </a>
          ))}
        </div>
      </div>
