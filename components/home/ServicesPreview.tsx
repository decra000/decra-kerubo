...
const tracks = [
  {
    ...
    distinction: "This track is about your company's legal surface — the contracts you sign, the data you collect, the IP you build, and the regulations you must navigate.",
    ...
    items: [
      ...
      { title: "Regulatory Mapping", body: "Understanding your legal landscape before you build or launch in any market." },
  ...
  },
  {
    ...
    distinction: "This track is about the founder as a person — how you structure your ownership, protect your equity, navigate tax decisions, and set up governance that doesn't come back to haunt you.",
...
const faqs = [
  { q: "What's the difference between the two tracks?", a: "Technology Law protects your company and product — IP, contracts, data, regulation. Founder Legal protects you as a person — your equity, structure, tax position, and governance rights. Some clients need both; many start with one." },
...
export function ServicesPreview({ intent }: { intent?: string | null }) {
  ...
  // Pre-select based on intent popup
  useEffect(() => {
      ...
      <div className="inner" ref={ref}>
        ...
        <div style={{ opacity: vis ? 1 : 0, transform: vis ? "none" : "translateY(20px)", transition: "all 0.6s ease", marginBottom: "4rem" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "5rem", alignItems: "end" }} id="svc-header-grid">
            <h2 className="t-display t-display-lg">Two tracks. One question: what needs protecting?</h2>
            ...
            return (
              ...
              >
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "1.25rem" }}>
                  ...
                  <ChevronDown
                    size={16}
                    style={{ color: isActive ? "var(--c-accent)" : "var(--c-ink-muted)", transform: isActive ? "rotate(180deg)" : "none", transition: "transform 0.3s, color 0.3s" }}
                  />
                </div>
                <p style={{ fontSize: "0.6rem", fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", color: "var(--c-accent)", marginBottom: "0.4rem", fontFamily: "var(--font-sans)" }}>{t.number}</p>
                ...
                <p style={{ fontSize: "0.8rem", color: isActive ? "rgba(245,237,216,0.45)" : "var(--c-ink-muted)", lineHeight: 1.65, fontFamily: "var(--font-sans)", transition: "color 0.3s", maxWidth: "28rem" }}>{t.tagline}</p>

                {/* Service pills — visible always */}
                <div style={{ display: "flex", flexWrap: "wrap", gap: "0.35rem", marginTop: "1.5rem" }}>
                  {t.offerings.slice(0, 3).map(o => (
                    <span key={o} style={{ fontSize: "0.6rem", padding: "0.2rem 0.55rem", borderRadius: "100px", border: `1px solid ${isActive ? "rgba(196,162,101,0.3)" : "var(--c-border-strong)"}`, color: isActive ? "rgba(245,237,216,0.5)" : "var(--c-ink-muted)", fontFamily: "var(--font-sans)", transition: "all 0.3s" }}>{o}</span>
                  ))}
                  <span style={{ fontSize: "0.6rem", padding: "0.2rem 0.55rem", borderRadius: "100px", border: `1px solid ${isActive ? "rgba(196,162,101,0.3)" : "var(--c-border-strong)"}`, color: isActive ? "rgba(196,162,101,0.7)" : "var(--c-ink-muted)", fontFamily: "var(--font-sans)", transition: "all 0.3s" }}>+{t.offerings.length - 3} more</span>
                </div>
              </button>
            );
          })}
        ...
        {/* ── Expanded detail panel — Tesla-style: slides in below ── */}
            ...
            return (
              <div style={{ padding: "3rem 2.75rem" }}>
                <div style={{ display: "grid", gridTemplateColumns: "2fr 3fr", gap: "4rem" }} id="detail-inner-grid">
                  ...
                  {/* Left — sticky summary */}
                  <div>
                    <p style={{ fontSize: "0.75rem", fontStyle: "italic", color: "var(--c-ink-muted)", lineHeight: 1.7, marginBottom: "2rem", fontFamily: "var(--font-sans)" }}>{t.distinction}</p>

                    <div style={{ display: "flex", flexWrap: "wrap", gap: "0.35rem", marginBottom: "2rem" }}>
                      {t.offerings.map(o => (
                        <span key={o} style={{ fontSize: "0.6rem", padding: "0.22rem 0.6rem", borderRadius: "100px", border: "1px solid var(--c-border-strong)", color: "var(--c-ink-muted)", fontFamily: "var(--font-sans)" }}>{o}</span>
                      ))}
                    </div>

                    {/* Deliverables */}
                    <p style={{ fontSize: "0.6rem", fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", color: "var(--c-ink-muted)", marginBottom: "0.75rem", fontFamily: "var(--font-sans)" }}>Deliverables</p>
                    <ul style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: "0.45rem", marginBottom: "2rem" }}>
                      {t.deliverables.map(d => (
                        <li key={d} style={{ display: "flex", alignItems: "flex-start", gap: "0.5rem", fontSize: "0.78rem", color: "var(--c-ink-mid)", fontFamily: "var(--font-sans)" }}>
                  ...
                  {/* Right — problem/solution + process + outcomes */}
                  <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                    {[{ label: "The Problem", text: t.problem }, { label: "The Approach", text: t.solution }].map(block => (
                      <div key={block.label} style={{ padding: "1.5rem", background: "var(--c-bg)", border: "1px solid var(--c-border)", borderRadius: "2px" }}>
                        ...
                        <p style={{ fontSize: "0.875rem", lineHeight: 1.8, color: "var(--c-ink-mid)", fontFamily: "var(--font-sans)" }}>{block.text}</p>
                      </div>
                    ))}
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
                      ...
                      <div style={{ padding: "1.5rem", background: "var(--c-bg)", border: "1px solid var(--c-border)", borderRadius: "2px" }}>
                        ...
                        <ol style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: "0.55rem" }}>
                          {t.process.map((step, i) => (
                            <li key={i} style={{ display: "flex", gap: "0.75rem", fontSize: "0.775rem", color: "var(--c-ink-mid)", fontFamily: "var(--font-sans)" }}>
                      ...
                      <div style={{ padding: "1.5rem", background: "var(--c-ink)", border: "1px solid var(--c-border)", borderRadius: "2px" }}>
                        ...
                        <ul style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: "0.55rem" }}>
                          {t.outcomes.map(o => (
                            <li key={o} style={{ display: "flex", alignItems: "flex-start", gap: "0.6rem", fontSize: "0.775rem", color: "rgba(245,237,216,0.55)", fontFamily: "var(--font-sans)" }}>
        ...
        {/* ── FAQs — always visible, compact accordion ── */}
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            {faqs.map((f, i) => (
              <div key={i} style={{ borderTop: "1px solid var(--c-border)" }}>
                ...
                >
                  <span style={{ fontSize: "0.875rem", fontFamily: "var(--font-sans)", fontWeight: 500, color: "var(--c-ink)", lineHeight: 1.5 }}>{f.q}</span>
                ...
                <div style={{ maxHeight: expandedFaq === i ? "200px" : "0", overflow: "hidden", transition: "max-height 0.35s ease" }}>
                  <p style={{ fontSize: "0.85rem", color: "var(--c-ink-muted)", lineHeight: 1.8, fontFamily: "var(--font-sans)", paddingBottom: "1.25rem", paddingRight: "2rem" }}>{f.a}</p>
                </div>
              </div>
            ))}
        ...
        <div style={{ marginTop: "4rem", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem", opacity: vis ? 1 : 0, transition: "opacity 0.6s ease 0.4s" }} id="svc-footer-grid">
          ...
          >
            <div>
              <p style={{ fontSize: "0.6rem", fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", color: "var(--c-accent)", marginBottom: "0.3rem", fontFamily: "var(--font-sans)" }}>Need AI Engineering?</p>
              <p style={{ fontSize: "0.8rem", color: "var(--c-ink-muted)", fontFamily: "var(--font-sans)" }}>Handled through <strong style={{ color: "var(--c-ink)" }}>Entrora Systems</strong></p>
            </div>
